...
import { Crown, Zap, Gem, Share2, Info, CheckCircle2, QrCode, Sparkles } from "lucide-react";
...
export function VirtualMembershipCard({ user }: VirtualMembershipCardProps) {
    ...
    return (
        <div className="space-y-4 pt-6 md:pt-8">
            <div className="flex flex-col md:flex-row items-end justify-between gap-8 px-4 border-b border-gray-100 pb-4">
                <div className="space-y-4 text-left">
                    <div className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-indigo-50 text-indigo-700 text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] shadow-sm border border-indigo-100/50 transition-all hover:bg-indigo-100">
                        <Sparkles size={14} />
                        Exclusive Member Identity
                    </div>
                    <div className="space-y-2">
                        <h2 className="text-4xl md:text-5xl font-black text-gray-900 tracking-tighter leading-none">
                            Your Digital <span className="text-indigo-600">Pass</span>
                        </h2>
                        <div className="flex items-center gap-3 text-gray-400 font-bold uppercase tracking-widest text-[11px]">
                            <div className="w-10 h-[2px] bg-indigo-500 rounded-full" />
                            Use for in-store priority & online perks
                        </div>
                    </div>
                </div>
            </div>

            <div className="flex flex-col items-center justify-center py-6 bg-gray-50/50 rounded-[4rem] border border-dashed border-gray-200">
                <div
                ...
                >
                    ...
                    >
                        ...
                        <div
                            ref={frontCardRef}
                            className={cn(
                                "js-mart-card-front absolute inset-0 w-full h-full rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-10 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.3)] z-20",
                            ...
                            <div className="relative h-full flex flex-col justify-between text-white z-10">
                                ...
                                <div className="flex justify-between items-end border-t border-white/10 pt-6">
                                    <div className="flex gap-10">
                                        ...
                                        <div className="space-y-0.5">
                                            <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] opacity-40">Valid Thru</p>
                        ...
                        <div
                            ref={backCardRef}
                            className="js-mart-card-back absolute inset-0 w-full h-full rounded-[2rem] md:rounded-[2.5rem] bg-[#111] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col"
                            style={{
                            ...
                            <div className="relative flex-1 px-8 md:px-12 py-6 flex flex-col gap-6">
                                ...
                                <div className="flex items-end justify-between mt-auto mb-4">
                                    <div className="space-y-3">
                                        <div className="space-y-1">
                                            <p className="text-[10px] text-gray-400 uppercase tracking-[0.2em] font-bold">Member ID</p>
                                            <p className="text-sm md:text-base text-white font-mono tracking-widest text-shadow-sm">
                                                {cardNumber}
                                            </p>
                                        </div>
                                        <p className="text-[9px] text-gray-600 max-w-[200px] leading-relaxed">
                ...
                <div className="mt-12 flex flex-col items-center gap-4">
                    <div className="flex items-center gap-3 px-6 py-3 bg-white shadow-xl rounded-full border border-gray-100 group hover:border-indigo-200 transition-all">
                        <div className="w-2 h-2 rounded-full bg-emerald-500" />
                        <span className="text-[11px] font-black text-gray-500 uppercase tracking-[0.2em] group-hover:text-gray-900">Tap Card to Flip & View Details</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { icon: CheckCircle2, title: 'Valid In-Store', desc: 'Flash card at checkout', color: 'text-emerald-600', bg: 'bg-emerald-50' },
                    { icon: Zap, title: 'Instant Sync', desc: 'Updates with your tier', color: 'text-indigo-600', bg: 'bg-indigo-50' },
                    { icon: Share2, title: 'Always Ready', desc: 'Available on any device', color: 'text-purple-600', bg: 'bg-purple-50' }
                ].map((item, i) => (
                    <div key={i} className="p-8 bg-white rounded-[2.5rem] border border-gray-100 flex items-center gap-6 shadow-sm hover:shadow-md transition-all group">
                        <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110", item.bg, item.color)}>
                            <item.icon size={28} />
                        </div>
                        <div>
                            <h4 className="font-black text-base text-gray-900 leading-tight">{item.title}</h4>
